...
Each pattern below follows the same structure: **symptom**, **why it hurts**, **fix**. They are grouped into four blocks, ordered from most to least severe: first the ones that can do irreversible damage, then the ones that crater quality and throughput, then context hygiene, and finally team scale and the agent's scope.

## Block 1 — High-stakes / irreversible

The anti-patterns in this block can do damage you can't undo: leaking data, breaking production, letting an attacker in. If you only have energy to fix five things on your team, make it these.

## 1. Corporate data in consumer-tier tools
...
## 2. Trusting tool output blindly
...
## 5. Autonomous orchestration without strong verification
...
## Block 2 — Quality and the work loop

These don't break production, but they crater throughput day to day and erode the team's trust in the agent. They're the ones that make people say "this is slower with AI than without it" — almost always because the loop isn't being closed.

## 6. Delegating without a verification loop
...
## 7. Reviewing agent PRs less carefully than human PRs
...
## 9. Heavy validation on every edit
...
## Block 3 — Context and knowledge hygiene

The symptoms here are slow and quiet: the agent "gets dumber" over time and you can't tell why. Almost always it's because its context has filled up with noise, or because the knowledge it needs exists but isn't discoverable.

## 11. The 2000-line AGENTS.md
...
## 12. Treating memory as a notebook
...
## 14. Project knowledge the agent can't discover
...
## Block 4 — Team scale and scope

The last three are "ceiling" anti-patterns: they don't stop you from getting started, but they stop you from growing. They're what separates a productive solo developer from a team that operates agents as shared infrastructure.

## 15. No team conventions
...
## 16. An MCP server for everything
